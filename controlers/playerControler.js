...
const getAll = (req, res) => {
  let sql =
    "SELECT player.idplayer, player.name,player.last_name, player.age, team.name as team_name\
  FROM player \
  LEFT JOIN team \
...
const create = (req, res) => {
  ...
  let sql =
    "INSERT INTO player (name,last_name,age, idteam)\
  VALUES (?,?,?,?)";
...
const update = (req, res) => {
  ...
  connection.query(
    sql,
    [name, last_name, age, idteam, idplayer],
    (err, result) => {
      if (err) throw err;
      res.send(result);
    }
